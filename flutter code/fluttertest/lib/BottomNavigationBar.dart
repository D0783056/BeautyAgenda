...
class _BottomNavigationBarExampleState
  ...
  Widget build(BuildContext context) {
    final _kTabPages = <Widget>[
      CameraScreen(context, id),MoodRecordPage(),HomePage(id),DaymenuPage(),Week(id)
    ];
    final _kBottmonNavBarItems = <BottomNavigationBarItem>[
      BottomNavigationBarItem(icon: Icon(Icons.camera, size: 35,), label: '膚況檢測'),
      BottomNavigationBarItem(icon: Icon(Icons.favorite,size: 35,), label: '心情追蹤'),
      BottomNavigationBarItem(icon: Icon(Icons.home,size: 40,), label: '主頁面'),
      BottomNavigationBarItem(icon: Icon(Icons.today,size: 35,), label: '每日養顏'),
      BottomNavigationBarItem(icon: Icon(Icons.date_range,size:35,), label: '每週養顏'),
    ];
